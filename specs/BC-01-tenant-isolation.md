...
| E-06 | The same mailbox address is connected to two workspaces | **Refused, and contained where it already exists.** Adding or restoring an address that is live on another workspace is rejected before anything is written, naming the other workspace (`findMailboxAddressConflicts`). The unique key is still per client, so for any pair already in the database only the workspace that had the address **first** stores raw inbound mail; the other keeps its own reply matching and bounce detection — both already require a matching outbound in the same client — and stops keeping a verbatim copy of an inbox that is not its own (`mayPersistRawInboundMail`, counted as `rawCopiesWithheld` in the sync audit row). Closed 2026-08-27; previously this read "Permitted" and the raw store, body text included, was duplicated |
