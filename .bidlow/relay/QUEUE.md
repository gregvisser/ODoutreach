...
Items 20-25 are those commitments and they OUTRANK everything below them.
Internal quality work waits.
...
| # | Item | Status |
|---|---|---|
| 20 | **DEADLINE 31 AUG — VERIFY OPEN TRACKING IS OFF IN PRODUCTION.** Greg told the client in writing that tracking is off and that it was the cause of the domain mismatch. `isOpenTrackingPixelEnabled()` returns TRUE unless `OPEN_TRACKING_PIXEL` is exactly `off`. Confirm the live Azure value. If it is anything but `off`, that is a broken written promise on the single point the client cares most about. Report the actual value. | DONE 11 — **VERIFIED `off`. The written promise HOLDS.** Live Azure app setting on `app-opensdoors-outreach-prod` reads exactly `off` (lower-case, no whitespace) — read 2026-08-26 via `az webapp config appsettings list`. `OPEN_TRACKING_REQUIRE_ALIGNED_DOMAIN` is unset, which is moot while the pixel is off globally. Three further checks, because a correct config value is not the same as a pixel that stops firing: (a) only TWO call sites embed a pixel (`execute-one.ts`, the Gmail and Graph legs) and BOTH route through `buildOpenTrackingPixelUrl`, so there is no bypass; (b) the compiled server bundle keeps a genuine RUNTIME read — `if("off"===process.env.OPEN_TRACKING_PIXEL)return null` — so the value is NOT inlined at build time from the GitHub Actions environment, which was the real risk of a cosmetic switch; (c) deployed commit `1f8a8e7` matches the inspected code. NOT verified: opens-stopped evidence from the production database — the DB firewall allows Azure services only, and opening a live client database to a workstation IP is Greg's call, not the relay's. **Defect found and fixed while verifying:** the switch failed OPEN — `!== "off"` meant `OFF`, `Off`, `off ` (trailing space), `false`, `0`, `no` all silently RESUMED tracking, with no error, no log and nothing on screen. One careless keystroke in the Azure portal, which has no validation, would have broken this exact written promise invisibly. Now normalised (trim + lower-case, fail-closed) with 11 red-first tests that were watched failing before the fix. | 
