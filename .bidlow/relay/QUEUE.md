...
| 5 | **Reply claiming** — Part 2 of `ALERTS-AND-CLAIMING.md`. Built as specified: **advisory, not a lock** — every button stays enabled and no send gate reads the claim. Opening a reply records who and when; the next person sees "Sarah Okafor opened this 2 minutes ago"; you are never shown your own claim; a claim stops showing at 30 minutes; replying / suppressing / marking handled clears **every** claim on the conversation. The two routes to a reply (`activity/messages/[messageId]` and `activity/replies/[replyId]`) resolve to the **same** claim subject, so one claim covers both. **New table `ReplyClaim`, additive-only migration `20260826120000_reply_claims`** — deliberately NOT `InboundMailboxMessage.metadata`, where the neighbouring handled-state lives: the reply sync's upsert rewrites `metadata` wholesale every 15 minutes, so a claim stored there would look wired up and silently stop firing. **PROVEN, not assumed:** 16 pure + 9 mocked unit tests watched RED first; **6 integration tests against real Postgres** (two real staff rows — Bob is shown Sarah's claim, Sarah is shown nothing, a back-dated row stops showing, a release clears both, no cross-workspace leak, cascade on workspace delete); migration applied to a real database and `migrate diff` reports **"No difference detected"**. A wiring test locks the three ways this could go quiet, and **all three were deliberately broken and watched go red** — which is how I found that my own first assertion passed against a renamed component, and tightened it. **Related finding, NOT fixed (out of scope, own queue item): the reply sync's wholesale `metadata` overwrite also erases the existing `handledAt` / `handledByStaffUserId` / reply history for recently-synced messages.** Gates: lint 0 errors, typecheck clean, 2260 unit + 100 integration tests green, build compiled. **PR #231 is open with BOTH CI checks green (verify + E2E Playwright) and is DELIBERATELY NOT MERGED — this is the one thing left and it is Greg's keystroke, not mine.** `PRODUCTION_PRISMA_MIGRATE` is `true`, so merging runs DDL against a paying client's live database, and the standing working agreement puts one approval gate before any schema change. The migration is additive-only and reversible, so this is caution rather than doubt — but an unattended overnight cycle is the wrong thing to be executing DDL on a client's production database. **Nothing is live yet: do not read DONE as deployed.** Merge PR #231 to ship it. | DONE 8 — PR #231 green, NOT merged (awaiting Greg: migration) |
...
| 14 | **The reply sync erases who handled a message.** Found while building item 5, NOT fixed there (different concern, and it touches the sync path). `mailbox-inbox-sync.ts` upserts each fetched message with `metadata: meta` in BOTH the `create` and the `update` branch — a wholesale overwrite of the JSON column. But `metadata.handling` is where `handledAt`, `handledByStaffUserId`, `lastRepliedAt` and `replyOutboundEmailIds` live (`inbound-message-handling.ts`, PR J). The sync pulls the newest `top` messages with no date filter and runs every 15 minutes, so for any recently-received message the "Handled by X" badge and the reply history are wiped shortly after they are written. Nobody would see an error: the screen simply reverts to "Unhandled". **Verify the blast radius first** (how many messages fall inside `top` on a real run), then merge `handling` through instead of overwriting — `mergeHandlingIntoMetadata` already exists and does exactly this. Needs a red-first test that syncs twice over a handled message and asserts the handled state survives. | TODO |
