...
| 3 | **Load speed — fix (code side only).** The measurement cleared the suspect: `loadClientWorkspaceBundle` is a CONSTANT 19 round-trips at 1/6/20 mailboxes, no N+1. Two small code items remain: `getAccessibleClientIds` scans the whole `Client` table unbounded on every page, and `ClientMailboxIdentity` is queried 4x per load when the bundle already holds it. Both are tidy-ups, NOT the cause. Read `docs/ops/LOAD-SPEED-MEASUREMENT.md` first. | TODO |
