...
Started 2026-08-31 08:48:12, took about 18.3 minutes.
...
* When you finish, update this item's row in `.bidlow/relay/QUEUE.md` to
  `DONE 172`, or back to `TODO` with a note if you could not do it.
...
Markdown bold around it is fine - `| **DONE 172 - ...** |` reads correctly.
...
* Finished it -> `DONE 172 - <what you did, and the proof>`
* Did some of it -> `PARTIAL 172 - <what is done, what is left>`. PARTIAL
...
`.bidlow/STATE.md` is updated and merged (`b1b958377bfda289f97aae405a5a755429ebb4a1`), working tree clean on `main`, no open PRs.

**Final state for this session:** row 138 re-verified and closed for the third time (`DONE 172`, merge `4480491`), state recorded (merge `b1b9583`). The recurring cause — `relay-watch.ps1` running a stale copy of itself — is documented in both `QUEUE.md` and `STATE.md` for Greg to act on with `relay-start.cmd`.
