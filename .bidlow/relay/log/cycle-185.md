...
Started 2026-08-31 11:36:17, took about 27.2 minutes.
...
* When you finish, update this item's row in `.bidlow/relay/QUEUE.md` to
  `DONE 185`, or back to `TODO` with a note if you could not do it.
...
Markdown bold around it is fine - `| **DONE 185 - ...** |` reads correctly.
...
* Finished it -> `DONE 185 - <what you did, and the proof>`
* Did some of it -> `PARTIAL 185 - <what is done, what is left>`. PARTIAL
...
`.bidlow/STATE.md` is updated and merged (`main` now at `5d410dc`), working tree clean, no open PRs.

## Summary

Addressed the stop-hook feedback: `.bidlow/STATE.md` hadn't been updated since cycle 182 (cycles 183/184 skipped it). Appended a cycle 185 entry in the file's established format covering what changed (re-verified the already-merged squash-merge guard fix, no rework; diagnosed the self-reopen root cause; flipped `delete_branch_on_merge` on), confirmed nothing is half-done, noted no one-way-door decisions, flagged the pick-up items for next session (check for another reopen before writing code; the fourteen dangling junk branches still awaiting cleanup), and confirmed nothing contradicts `PROJECT.json`. Merged via PR #496 (`5d410dc`), green CI, clean exit.
