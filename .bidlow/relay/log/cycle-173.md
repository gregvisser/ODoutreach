...
Started 2026-08-31 09:07:33, took about 9.8 minutes.
...
* When you finish, update this item's row in `.bidlow/relay/QUEUE.md` to
  `DONE 173`, or back to `TODO` with a note if you could not do it.
...
Markdown bold around it is fine - `| **DONE 173 - ...** |` reads correctly.
...
* Finished it -> `DONE 173 - <what you did, and the proof>`
* Did some of it -> `PARTIAL 173 - <what is done, what is left>`. PARTIAL
...
Merged and confirmed. Merge commit `76719abe7a9efe96c63247f797acf483829f4510` on `main`, verified equal to `origin/main` via `git ls-remote`.

## Cycle 173 summary

Row 138 (deck out-of-order headline) was handed out a **fourth** cycle running as `IN PROGRESS`, despite being closed on `main` by cycles 169–172. Checked `main` first per this repo's standing rule — found it already merged and re-verified three times over. Re-verified everything from scratch rather than trusting the record:

- `estateOutOfOrder`/`outOfOrderHeadline`/`.headline-ooo` present and wired into `bidlow-deck.mjs`'s `render()`, dated backup intact
- Real test file (`standards/bidlow-deck-out-of-order-headline.test.ts`): 2/2 passed
- `npm run lint`: 0 · `npm run typecheck`: 0 · `queue-file-integrity.test.ts`: 9/9
- `docs/ops/DECK-OUT-OF-ORDER-HEADLINE-2026-08-31-cycle169.md` present with before/after evidence

No code change was needed — closed `DONE 173`, merged PR #482 (`76719ab`).

The real finding: **the queue keeps re-dispatching this same closed row**, most likely because `relay-watch.ps1` is running a stale in-memory copy of itself (identified back in cycle 172 — loaded `51AF85ED01BF`, disk has `E97F4D42A323`). That's not fixable from inside this row (not the authorized file, and a watcher fix is inert until restarted). Four cycles have now gone to re-verifying zero net change — restarting the watcher via `relay-start.cmd` is flagged as the highest-value action outstanding.
