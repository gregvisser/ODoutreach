...
Started 2026-08-31 14:54:06, took about 25.1 minutes.
...
* When you finish, update this item's row in `.bidlow/relay/QUEUE.md` to
  `DONE 192`, or back to `TODO` with a note if you could not do it.
...
Markdown bold around it is fine - `| **DONE 192 - ...** |` reads correctly.
...
* Finished it -> `DONE 192 - <what you did, and the proof>`
* Did some of it -> `PARTIAL 192 - <what is done, what is left>`. PARTIAL
...
Row 134 is complete. Summary of this cycle's work:

**Answered all four of Greg's questions with code-level evidence**, written up in `docs/ops/ROW134-FOUR-QUESTIONS-2026-08-31-cycle192.md`:
1. **Universe → sequence**: list creation works and is well-surfaced; sequence creation is a separate, unlinked flow — no CTA connects them.
2. **Send cooldown**: real and tested at planning time (10 days); the dispatch-time re-check is built and tested but disabled by default.
3. **Training modules**: 12 confirmed drift defects vs. live code — worst is a regressed `{{email_signature}}` in the worked example, the exact bug the module warns against two paragraphs later.
4. **AI ask-box**: feasible and cheap (~$1–30/month) but only CR-10-safe if scoped strictly to static training content, never a general support box.

Raised four follow-up rows (146–149) for the real fixes, per the brief's "build nothing here" instruction. Merged to `main` as `dab1019` (PR #505) plus a same-cycle hash-record follow-up (`a03dd29`, PR #506), both confirmed via `git ls-remote`. No open PRs remain. QUEUE.md row 134 is `DONE 192`.
