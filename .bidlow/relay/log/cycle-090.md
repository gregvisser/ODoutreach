...
Started 2026-08-29 06:44:11, took about 38.8 minutes.
...
* When you finish, update this item's row in `.bidlow/relay/QUEUE.md` to
  `DONE 90`, or back to `TODO` with a note if you could not do it.
...
Markdown bold around it is fine - `| **DONE 90 - ...** |` reads correctly.
...
* Finished it -> `DONE 90 - <what you did, and the proof>`
* Did some of it -> `PARTIAL 90 - <what is done, what is left>`. PARTIAL
...
`.bidlow/STATE.md` is written and merged (`26b6968`). Board clean, tree clean.

## What the STATE entry records

**Built:** item (5) of row 80 — AI-chosen send times, merged `cb0d227`, deployed and verified by hash on the direct App Service URL.

**Half-done, and where:** row 80 is `PARTIAL 90`, items 1–5 of 7. Remaining are (6) the rep performance dashboard and (7) best-message-by-job-title. The relay picks the row straight back up.

**The decision, and the one-way-door check:** the brief asked for "AI-chosen send times", which taken literally is a scheduler — and nothing in this application decides when mail leaves. Building it literally would have meant creating a scheduler *and* handing the model the dispatch clock in a single change. So the model advises and a person acts, enforced by a tool schema that has no field capable of expressing a schedule. Explicitly **not** a one-way door: additive migration only, no existing table or column touched, no client data moved, no send path added.

**What the next session should do first:** before building item (6), settle the reply-linkage hole deliberately rather than inheriting it. A reply only counts when it links to a send, and Gmail's Message-ID rewriting means some never link. Cycle 90 excluded unlinked replies because they have no send time to attribute. In a *rep* dashboard the same hole has more consequence — an unlinked reply has no rep either, so a rep whose replies happen to link less often would look worse at their job.

**Nothing contradicts PROJECT.json** — the hard rule was never approached: no send, no delete, no client data.

Two items remain yours and neither moved: verifying the per-token prices (six consecutive cycles blocked by an unavailable network) and setting `ANTHROPIC_API_KEY` in Azure.
