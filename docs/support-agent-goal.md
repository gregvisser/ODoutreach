...
2. Read OPEN tickets through support:list. Review each selected ticket and its attachments through support:get, retaining private output as described above. For mailbox or signature issues, run support:inspect-mailbox-signature with --client-slug or --mailbox-id (read-only; never sends mail) before guessing. Use `storedSignatureHealthy`, `supportConclusion`, and `storedImageUrls` in the JSON — when healthy, report “stored signature healthy; recipient render unconfirmed” because Activity shows plain-text snapshots only, not sent HTML. Do not recommend signature edits when `proposedFixes` is empty. Leave legacy IN_REVIEW, APPROVED and REJECTED states untouched. Review AWAITING_APPROVAL only when a separate instruction supplies the required decision.
