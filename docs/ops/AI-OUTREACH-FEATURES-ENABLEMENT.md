...
The only Anthropic HTTP client is `src/server/ai/anthropic-messages.ts`.
...
| `ANTHROPIC_API_KEY` | Azure App Setting **secret** | Non-empty workspace-scoped key | Unset/empty → `no_api_key` refusal, cost 0 | Empty in example |
| `ANTHROPIC_WORKSPACE_ID` | Azure App Setting (not a secret) | Set **only** if the key is identity-linked and Anthropic returns HTTP 400 `anthropic-workspace-id is required` | Unset → header omitted (correct for a key created inside one workspace) | Empty in example |
...
- **CR-10:** `carriesPersonalData: true` and Anthropic is **not** in `COVERED_PROCESSORS`. `runMeteredAiCall` refuses `no_processor_allowance` **even with a valid API key**. Enabling outreach flags does **not** start sending prospect reply text to Anthropic. Closing that is a DPA / `COVERED_PROCESSORS` decision, not this enablement.
...
1. Confirm `ANTHROPIC_API_KEY` is present (name check). If the live key is identity-linked, set `ANTHROPIC_WORKSPACE_ID`; if it was created inside workspace `ODoutreach`, leave the workspace id unset.
...
| 7 | REPLY_CLASSIFICATION | `/replies` | Still **Not checked yet** / unclassified for new replies while CR-10 holds. | No prospect body reaching Anthropic (`no_processor_allowance` on ledger if a call was attempted). |
...
**Still blocked by design after outreach opt-in:** reply classification (CR-10 / Anthropic DPA). That is not an outreach-flag bug.
