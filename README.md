...
Internal cold outreach operations platform for **OpensDoors** staff. One OpenDoors operating organisation, with separate client workspaces for data and mailbox scoping. Independent customer organisations are not the product target. This product focuses on **outreach execution** — sends, replies, suppression, and reporting — **not** CRM pipelines, deal stages, or lead scoring.

## What the codebase includes

- **Next.js 16** (App Router) + **TypeScript** + **Tailwind CSS v4** + **shadcn/ui**
- **Microsoft Entra ID** authentication via **Auth.js / NextAuth** (MFA enforced in Entra, not in-app). **Microsoft sign-in alone does not grant app access** — the user must have a **`StaffUser`** row with a provisioned Microsoft identity or recorded guest identity. Only pending legacy invitations without a recorded guest identity can bind once by normalized email; existing identities cannot be replaced through an email match. Owner accounts require an explicitly provisioned identity. Use **Entra enterprise app assignments** (users/groups), optional **`STAFF_EMAIL_DOMAINS`**, and client data access flows through the staff gate and live-client scoping helpers. Ordinary staff can access live client workspaces; owner-only operations are checked separately.
...
**Connection:** Use **Connect** to run provider OAuth in a separate window. Refresh tokens are stored **encrypted** in `MailboxIdentitySecret` (key material via `MAILBOX_OAUTH_SECRET` or `AUTH_SECRET`). **Microsoft 365:** an authorised operator may sign in with a **delegate/admin** account; the app checks that account can open the **declared** mailbox in Graph (`Mail.Read.Shared` / `Mail.Send.Shared` + Exchange mailbox permissions) and sends as that row via `/users/{mailbox}/…`. **Google:** the signing-in Google user must be able to call Gmail as the row (usually the same mailbox account; domain-wide delegation is an org-level prerequisite if you need a different model). **Disconnect** removes stored tokens. Connection establishes lifecycle and readiness only.
