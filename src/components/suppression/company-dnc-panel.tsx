...
export function CompanyDncPanel({ clientId, data }: { clientId: string; data: Awaited<ReturnType<typeof loadCompanyDncPage>> }) {
  ...
  function run(action: () => Promise<{ ok: boolean; error?: string; message?: string }>) {
    ...
    startTransition(async () => {
      try { const result = await action(); setMessage(result.ok ? result.message ?? "Saved." : result.error ?? "Could not save."); }
      catch { setMessage("Could not confirm the result. Refresh the page before trying again."); }
    });
  }
  return <section className="space-y-5 rounded-xl border p-5" aria-label="Company-name do-not-contact">
    ...
    <div className="space-y-2">
      ...
      {preview && <div className="space-y-2">
        ...
        {preview.errors.length ? <p role="alert">{preview.errors.slice(0, 5).map(error => `Row ${error.row}: ${error.message}`).join(" ")}</p> : <>
          ...
          <Button type="button" disabled={pending} onClick={() => run(() => importCompanyDncAction({ clientId, text, format }))}>Add company names</Button>
        </>}
      </div>}
    </div>
      ...
      {data.contacts.map(contact => <div key={contact.id} className="space-y-2 rounded border p-3">
        ...
        {contact.matches.map(entry => <div key={entry.id} className="flex flex-wrap items-center gap-2">
          ...
          {contact.decision.outcome === "REVIEW" && contact.company && <>
            <Button size="sm" variant="outline" disabled={pending} onClick={() => run(() => reviewCompanyDncAction({ clientId, contactId: contact.id, company: contact.company!, entryId: entry.id, outcome: "BLOCK" }))}>Same company — block</Button>
            <Button size="sm" variant="outline" disabled={pending} onClick={() => run(() => reviewCompanyDncAction({ clientId, contactId: contact.id, company: contact.company!, entryId: entry.id, outcome: "ALLOW" }))}>Different company — allow this match</Button>
          </>}
        </div>)}
      </div>)}
    ...
    {data.heldTotal > 0 && <div className="space-y-2">
      ...
      {data.heldEmails.map(email => <div key={email.id} className="flex flex-wrap items-center gap-3"><span>{email.toEmail} — {email.subject}</span><Button variant="outline" disabled={pending} onClick={() => run(() => retryCompanyDncHoldAction({ clientId, outboundEmailId: email.id }))}>Retry held email</Button></div>)}
