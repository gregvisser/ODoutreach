...
export function buildClientWorkflowSteps(input: ClientLaunchSnapshotInput): ClientWorkflowStep[] {
  ...
  // "Complete" is the same predicate the readiness rail and the launch gate
  // use — a mailbox alone never completes the Outreach step.
  const outreachComplete = isOutreachModuleReady(input);
