...
import { addCompanyNames, decideCompanyName, loadCompanyDncPage } from "@/server/suppression/company-names";
...
const pageFields = { page: z.number().int().nonnegative().optional(), heldPage: z.number().int().nonnegative().optional() };
const importSchema = z.object({ clientId: z.string().min(1), text: z.string().max(1_000_000), format: z.enum(["text", "csv"]), ...pageFields });
export async function importCompanyDncAction(input: z.infer<typeof importSchema>) {
  ...
  return { ok: true as const, message: `Added ${result.added} company names. ${result.duplicates} already listed or repeated.${warning}`, data: await loadCompanyDncPage(input.clientId, parsed.data.page ?? 0, parsed.data.heldPage ?? 0) };
}

const reviewSchema = z.object({ clientId: z.string().min(1), contactId: z.string().min(1), company: z.string().min(1).max(300), entryId: z.string().min(1), outcome: z.enum(["ALLOW", "BLOCK"]), ...pageFields });
export async function reviewCompanyDncAction(input: z.infer<typeof reviewSchema>) {
  ...
  refreshPages(input.clientId);
  return { ok: true as const, message: `Review saved. Existing email and domain blocks still apply.${warning}`, data: await loadCompanyDncPage(input.clientId, parsed.data.page ?? 0, parsed.data.heldPage ?? 0) };
}

const retrySchema = z.object({ clientId: z.string().min(1), outboundEmailId: z.string().min(1), ...pageFields });
export async function retryCompanyDncHoldAction(input: z.infer<typeof retrySchema>) {
  ...
  refreshPages(input.clientId);
  return result.count ? { ok: true as const, message: "Email queued again. Current sending limits and all blocking checks still apply.", data: await loadCompanyDncPage(input.clientId, parsed.data.page ?? 0, parsed.data.heldPage ?? 0) } : { ok: false as const, error: result.error ?? "The email changed. Refresh before trying again." };
}
