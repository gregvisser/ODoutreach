...
import { AUTOMATED_SEND_HELD_MESSAGE, automaticSequenceHoldReason, isAutomatedSequenceSend } from "@/lib/email-sequences/send-origin";
...
export async function beginOutboundDispatch(row: OutboundEmail, rfc822MessageId?: string, reconcilingAcceptedSend = false) {
  return prisma.$transaction(async (tx) => {
    ...
    if (!reconcilingAcceptedSend && isAutomatedSequenceSend(current.metadata)) {
      ...
      const clients = await tx.$queryRaw<Array<{ autonomousSendEnabled: boolean | null; serviceTier: string | null; deletedAt: Date | null; status: string }>>`
        SELECT "autonomousSendEnabled", "serviceTier", "deletedAt", status FROM "Client"
        WHERE id = ${current.clientId} FOR SHARE`;
      const client = clients[0];
      const holdReason = !client || client.deletedAt || client.status !== "ACTIVE" ? AUTOMATED_SEND_HELD_MESSAGE : automaticSequenceHoldReason(client);
      if (holdReason) {
        await tx.outboundEmail.update({ where: { id: current.id }, data: {
          ...
          nextRetryAt: null, lastErrorCode: "AUTOMATED_SEND_DISABLED", lastErrorMessage: holdReason,
          failureReason: holdReason,
        } });
        await markReservationReleasedForOutboundInTransaction(tx, current.id);
        return { ok: false as const, error: holdReason };
      }
    }
    let now = new Date();
