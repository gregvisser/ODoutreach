...
/**
 * How many completed UTC days has this mailbox actually SENT on?
 *
 ...
 *
 * Excludes the current day: its first accepted send must not increase the
 * allowance for the rest of that same day. Counted as DISTINCT UTC DATES
 * rather than total sends, so the ramp keeps the
